...
class Model(sb.Model):
    ...
    def __init__(self, **kwargs):
        ...
        # backward scanning from pos nt to nt - 1
        for nt in range(1, self.l_mrna):
            backward_scan(model, tc=self.tc, ssu=self.ssu, lsu=self.lsu,
                 mrna=self.mrna, pos=nt, k=k_backward_scan)
        ...
        for nt in (list(range(int(uorf1_start.value), int(uorf1_stop.value), 3)) +
            ...
            if nt == x_stall.value:
                elongate(model, ssu=self.ssu, lsu=self.lsu, mrna=self.mrna,
        ...
        from .reactions.ribosome_collision import (
            ...
            collide_upon_backward_scanning,
        )
        ...
        for nt in range(self.l_mrna):
            ...
            # scanning ribosomes collide with trailing obstacles at the rate at which they backward scan
            if nt > self.l_ssu:
                collide_upon_backward_scanning(model, ssu=self.ssu, lsu=self.lsu, mrna=self.mrna, pos=nt, k=k_backward_scan)
        ...
        from .reactions.scanning_premature_termination import (
            ...
            scan_terminate_no_hit_tc_ejects,
            scan_terminate_3_hit_elongating_tc_ejects,
            scan_terminate_3_hit_scanning_tc_ejects,
            scan_terminate_5_hit_elongating_tc_ejects,
            scan_terminate_5_hit_scanning_tc_ejects,
            scan_terminate_both_hit_elongating_elongating_tc_ejects,
            scan_terminate_both_hit_elongating_scanning_tc_ejects,
            scan_terminate_both_hit_scanning_elongating_tc_ejects,
            scan_terminate_both_hit_scanning_scanning_tc_ejects,
        )
       ...
       # scanning ribosome terminates if it reaches the end of mRNA, either with or without 5' collision with tc ejection
        scan_terminate_no_hit_tc_ejects(model, ssu=self.ssu, mrna=self.mrna, pos=self.l_mrna-1, tc=self.tc, k=k_scan_term_3_end)
        scan_terminate_5_hit_elongating_tc_ejects(model, ssu=self.ssu, lsu=self.lsu, mrna=self.mrna, tc=self.tc, 
            pos=self.l_mrna-1, k=k_scan_term_3_end)
        scan_terminate_5_hit_scanning_tc_ejects(model, ssu=self.ssu, mrna=self.mrna, tc=self.tc,
            pos=self.l_mrna-1, k=k_scan_term_3_end)

        # abortion by scanning and elongating ribosomes with tc ejection
        for nt in range(self.l_mrna - 1):
            scan_terminate_no_hit_tc_ejects(model, ssu=self.ssu, mrna=self.mrna, tc=self.tc, 
            pos=nt, k=k_scan_term_no_hit)

        # abortion by scanning ribosomes that hit a leading elongating ribosome with tc ejection
        for nt in range(self.l_mrna - self.l_ssu):
            scan_terminate_3_hit_elongating_tc_ejects(model, ssu=self.ssu, lsu=self.lsu, mrna=self.mrna, tc=self.tc,  
                pos=nt, k=k_scan_term_3_hit_80s)

        # abortion by scanning ribosomes that hit a leading scanning ribosome with tc ejection
        for nt in range(self.l_mrna - self.l_ssu):
            scan_terminate_3_hit_scanning_tc_ejects(model, ssu=self.ssu, mrna=self.mrna, tc=self.tc,  
            pos=nt, k=k_scan_term_3_hit_40s)

        # abortion by scanning ribosomes that hit (or are hit by) a trailing elongating ribosome with tc ejection
        for nt in range(self.l_ribo, self.l_mrna - 1):
            scan_terminate_5_hit_elongating_tc_ejects(model, ssu=self.ssu, lsu=self.lsu, mrna=self.mrna, tc=self.tc,  
                pos=nt, k=k_scan_term_5_hit_80s)

        # abortion by scanning ribosomes that hit (or are hit by) a trailing scanning ribosome with tc ejection
        for nt in range(self.l_ssu, self.l_mrna - 1):
            scan_terminate_5_hit_scanning_tc_ejects(model, ssu=self.ssu, mrna=self.mrna, tc=self.tc,  
            pos=nt, k=k_scan_term_5_hit_40s)

        # abortion by scanning ribosomes that hit a leading elongating ribosome and trailing elongating ribosome with tc ejection
        for nt in range(self.l_ribo, self.l_mrna - self.l_ssu):
            scan_terminate_both_hit_elongating_elongating_tc_ejects(model, ssu=self.ssu, lsu=self.lsu, mrna=self.mrna, tc=self.tc,  
                pos=nt, k=k_scan_term_both_hit_80s_80s)

        # abortion by scanning ribosomes that hit a leading scanning ribosome and trailing elongating ribosome with tc ejection
        for nt in range(self.l_ribo, self.l_mrna - self.l_ssu):
            scan_terminate_both_hit_elongating_scanning_tc_ejects(model, ssu=self.ssu, lsu=self.lsu, mrna=self.mrna, tc=self.tc,  
                pos=nt, k=k_scan_term_both_hit_80s_40s)

        # abortion by scanning ribosomes that hit a leading elongating ribosome and trailing scanning ribosome with tc ejection
        for nt in range(self.l_ssu, self.l_mrna - self.l_ssu):
            scan_terminate_both_hit_scanning_elongating_tc_ejects(model, ssu=self.ssu, lsu=self.lsu, mrna=self.mrna, tc=self.tc,  
                pos=nt, k=k_scan_term_both_hit_40s_80s)

        # abortion by scanning ribosomes that hit a leading scanning ribosome and trailing scanning ribosome with tc ejection
        for nt in range(self.l_ssu, self.l_mrna - self.l_ssu):
            scan_terminate_both_hit_scanning_scanning_tc_ejects(self, ssu=self.ssu, mrna=self.mrna, tc=self.tc,  
                pos=nt, k=k_scan_term_both_hit_40s_40s)