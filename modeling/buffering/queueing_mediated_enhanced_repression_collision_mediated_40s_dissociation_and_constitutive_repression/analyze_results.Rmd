...
# Investigate length bias for ivanov/geballe
...
plot_data <- data %>%
  filter(k_elong_stall == 0.001,
         ...
         uorf_reinit == 50,
         l_scan_capture == 0) %>%
  mutate(model = if_else(k_abort == 0, "Ivanov", "Geballe")) %>%
  print()

model_labeller <- function(x) paste0("Model : ", x, "")
queue_labeller <- function(x) paste0("queue size: \n", x, " footprints")

plot_data %>% 
  ggplot(aes(x = log2(k_cap_bind), y = log2(psr), color = as.factor(uorf_init))) +
  facet_wrap(~ model + queue_size, ncol = 4, labeller = labeller(model = model_labeller, queue_size = queue_labeller)) +
  geom_point(size = 1) +
  geom_line() +
...
ggsave('figures/ivanov_geballe_length_bias.pdf', limitsize = FALSE)
