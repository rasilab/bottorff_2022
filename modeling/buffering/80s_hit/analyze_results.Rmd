...
param_data <- annotations %>%
  mutate(uorf_init = round(100*k_start_uorf2 / (5 + k_start_uorf2)),2,
         ...
         uorf_reinit = round(100*5/(k_terminated_ssu_recycle_uorf2 + 5),1)) %>%
...
ggsave('figures/andreev_buffering_main.pdf', limitsize = FALSE)

```

# With control matched params and a stall (with d_stall disallowing Ivanov), Andreev buffering ~abolished (reinit too high)
...
ggsave('figures/andreev_buffering_control_matched.pdf', limitsize = FALSE)

```

# Andreev acquires ivanov buffering when an elongation stall is present and baseline init is low (to allow for increase)
...
ggsave('figures/andreev_buffering_supp_queueing.pdf', limitsize = FALSE)

```

# Plot PSR as a function of initiation rate for different k_scan = k_elong speeds. For whatever reason, raising these speeds (equally) reduces buffering. Abortive termination rate is very high (200/s), so that shouldn't be close enough to the speeds in question (2 & 5) to make a difference in competition between rates. uORF init = 80%, uORF reinit = 2% (high init and low reinit needed for buffering). No elongation stall here, and Ivanov behavior wouldn't be very relevant here because uORF initiation is already so high (80%, not much room to grow)
...
k_abort_labeller <- function(x) paste0("dissociation rate: ", x, " s-1")

plot_data %>% 
  ggplot(aes(x = log2(k_cap_bind), y = log2(psr), color = as.factor(k_scan))) +
  #facet_wrap(~ k_abort, ncol = 5, labeller = labeller(k_abort = k_abort_labeller)) +
  geom_point(size = 1) +
  geom_line() +
  viridis::scale_color_viridis(discrete = T, end = 0.9) +
  scale_y_continuous(breaks = scales::pretty_breaks(n=3)) +
  scale_x_continuous(breaks = scales::pretty_breaks(n=4)) +
  labs(x = "ribosome loading (log2, s-1)", y = "main ORF protein\noutput (log2, s-1)",
       color = "k_scan & k_elong (codons/s)")

ggsave('figures/andreev_buffering_supp_speed.pdf', limitsize = FALSE)
...
ggsave('figures/andreev_buffering_supp_length.pdf', limitsize = FALSE)

```

# Conclusions

 - preferred translation in the 80S-hit model requires several conditions
 1) rare continued scanning following termination at the uORF
 2) a well-initiating uORF
 - preferred translation also benefits from longer uORFs
 - preferred translation is lost when parameters matched to experimental control data are used (uORF doesn't initiate well enough, continued scanning is too frequent, short uORF) whether or not an elongation stall is present
 - when an elongation stall is included in the model, d_stall-dependent preferred translation is acquired only when d_stall equals an integer multiple of the ribosome footprint (30 nt here), similar to that in the queueing-mediated enhanced repression model