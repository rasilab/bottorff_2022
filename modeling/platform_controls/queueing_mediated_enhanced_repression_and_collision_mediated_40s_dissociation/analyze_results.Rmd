---
title: "Ivanov and Andreev models with finely tuned initiation and varying reinitiation"
...
- I had decreased the initiation rate for WT uORF2 from k_start_uorf2 = 0.5 to
0.2. However, this seems to make the fit significantly worse below compared to
to the previous fit [here](https://github.com/rasilab/tybottorffdocs/blob/e9da429/modeling/simulation_runs/0a_ivanov_geballe_models_vary_reinit_init_for_fitting_controls/analyze_results.md).
- So we might want to do a broader parameter sweep to identify right values. If not,
we can think about reverting to k_start_uorf2 = 0.5.
- We also want to decrease the k_start_uorf2 to 0.1 since that will give 2% initiation
frequency which will be 20-30-fold lower than the 50% initiation efficiency of strong Kozak.
...
# Plot PSR as a function of initiation rate for different rates of uORF2 initiation and scan abortive termination for 50% reinitiation
...
```{r}
expt_data <- read_csv("/fh/fast/subramaniam_a/user/tbottorf/analysis/luciferase_assays/20190422_exp2_ul4_mutants/tables/nluc_fluc_control_mutants.csv") %>% 
