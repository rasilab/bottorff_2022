...
ggsave('figures/ivanov_d_stall_back_scan_buffering.pdf', limitsize = FALSE)
...
subset_rxns_uorf <- rxn_data %>% 
  filter(str_detect(name, "^(scan_[:digit:]|scan_from_|scan_to|backwards_scan)")) %>% 
...
uorf2_n_data_25 <- uorf2_init_data %>%
  mutate(total = scan_25 + scan_to_elongate_25 + scan_to_elongate_from_leading_collision_25 + backwards_scan_25 + backwards_scan_from_scan_collision_25 + backwards_scan_from_elongation_collision_25) %>% 
...
plot_data <- uorf2_n_data_25 %>%
  filter(k_scan_term_3_hit_80s == 0) %>%
  print()

d_stall_labeller <- function(x) paste0("d_stall: ", x, " nt")

plot_data %>% 
  ggplot(aes(x = log2(k_cap_bind), y = log2(init_fraction / uorf_init), color = as.factor(k_backwards_scan))) +
  facet_wrap(~ d_stall, ncol = 3, labeller = labeller(d_stall = d_stall_labeller)) +
  geom_point(size = 1) +
  geom_line() +
  viridis::scale_color_viridis(discrete = T, end = 0.9) +
  ...
  scale_x_continuous(breaks = scales::pretty_breaks(n=3)) +
  theme(axis.text.y = element_text(size = 7), axis.text.x = element_text(size = 7)) +
  labs(x = "ribosome loading (log2, s-1)", y = "uORF initiation\n(real / baseline, log2)",
       color = "backwards\nscanning\nrate (nt/s)")

ggsave('figures/ivanov_d_stall_back_scan_uorf_init.pdf', limitsize = FALSE)
